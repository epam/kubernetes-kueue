...
| `kueue_pending_workload_max_wait_time_seconds` | Gauge | 该 ClusterQueue 分桶内待处理工作负载排队等待时间的最大值（秒）。<br>排队等待时间为自工作负载创建或上次重新排队至当前的时长。<br>`status` 与 kueue_pending_workloads 一致：`active` 表示准入堆栈中的工作负载（含正在调度的 inflight）；<br>`inadmissible` 表示准入失败后等待集群条件变化的工作负载。<br>当 ClusterQueue 对调度不可用时，`active` 会被合并计入 `inadmissible`。<br>若某分桶没有待处理工作负载，则不导出该时间序列。 | `cluster_queue`: ClusterQueue 的名称<br> `status`: `active` 或 `inadmissible`（与 kueue_pending_workloads 相同）<br> `replica_role`: `leader`、`follower` 或 `standalone` 其中之一 |
| `kueue_pending_workload_mean_wait_time_seconds` | Gauge | 该 ClusterQueue 分桶内待处理工作负载排队等待时间的平均值（秒）。<br>语义与 kueue_pending_workload_max_wait_time_seconds 一致。 | `cluster_queue`: ClusterQueue 的名称<br> `status`: `active` 或 `inadmissible`（与 kueue_pending_workloads 相同）<br> `replica_role`: `leader`、`follower` 或 `standalone` 其中之一 |
