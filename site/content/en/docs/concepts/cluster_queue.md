...
{{% alert title="Note" color="primary" %}}
`FlavorFungibilityImplicitPreferenceDefault` remains available as a transition aid.
It defaults to `true` in releases v0.15 and v0.16, implicitly applying
`PreemptionOverBorrowing` whenever `preference` is unset, and is scheduled for removal
in v0.17.
{{% /alert %}}
