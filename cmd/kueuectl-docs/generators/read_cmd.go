...
func GetSpec() KueuectlSpec {
	// Initialize a kueuectl command that we can use to get the help documentation
	kueuectl := app.NewDefaultKueuectlCmd()

	// Create the structural representation
	return NewKueuectlSpec(kueuectl)
}

func NewKueuectlSpec(c *cobra.Command) KueuectlSpec {
	return KueuectlSpec{
