...
var (
	GenKueuectlDir = flag.String("gen-kueuectl-dir", "generators", "Directory containing kueuectl files")
)

func getTocFile() string {
	return filepath.Join(*GenKueuectlDir, "toc.yaml")
}

func getStaticIncludesDir() string {
	return filepath.Join(*GenKueuectlDir, "static_includes")
}
...
func GenerateFiles() error {
	...
	manifest := &Manifest{}
	manifest.Title = "Kueuectl Reference Docs"
	manifest.Copyright = "<a href=\"https://github.com/kubernetes-sigs/kueue\">Copyright 2024 The Kubernetes Authors.</a>"

	NormalizeSpec(&spec)

	if err := os.MkdirAll(*GenKueuectlDir+"/includes", os.FileMode(0700)); err != nil {
		return err
	}
...
func NormalizeSpec(spec *KueuectlSpec) {
...
func WriteManifest(manifest *Manifest) error {
	...
	jsonfile, err := os.Create(*GenKueuectlDir + "/" + JsonOutputFile)
	if err != nil {
...
func WriteCommandFiles(manifest *Manifest, toc ToC, params KueuectlSpec) error {
	...
	err = filepath.Walk(getStaticIncludesDir(), func(path string, info os.FileInfo, err error) error {
		if !info.IsDir() {
			to := filepath.Join(*GenKueuectlDir, "includes", filepath.Base(path))
	...
	if len(missingCommands) > 0 {
		return fmt.Errorf("kueuectl commands %v are missing from table of contents", sets.List(sets.KeySet(missingCommands)))
	}
...
func WriteCategoryFile(c Category) error {
	...
	fn := strings.ReplaceAll(c.Name, " ", "_")
	f, err := os.Create(*GenKueuectlDir + "/includes/_generated_category_" + strings.ToLower(fmt.Sprintf("%s.md", fn)))
	if err != nil {
...
func WriteCommandFile(manifest *Manifest, t *template.Template, params TopLevelCommand) error {
	...
	f, err := os.Create(*GenKueuectlDir + "/includes/_generated_" + strings.ToLower(params.MainCommand.Name) + ".md")
	if err != nil {
		return fmt.Errorf("failed to open index: %w", err)
	}
	defer f.Close()
