...
func TestRecordCohortMetrics_QuotaHierarchyLikeIntegration(t *testing.T) {
	...
	setupRecordMetricsHierarchy(t, ctx, log, cache,
		[]*kueue.ResourceFlavor{
...
func (f cohortMetricsFixture) setup(t *testing.T, flavors []*kueue.ResourceFlavor, cohorts []*kueue.Cohort, cqs []*kueue.ClusterQueue) {
	t.Helper()
	setupRecordMetricsHierarchy(t, f.ctx, f.log, f.cache, flavors, cohorts, cqs)
}
...
func addTestUsage(cache *Cache, fr []usageChange) {
	for _, val := range fr {
		cache.hm.ClusterQueue(kueue.ClusterQueueReference(val.cqName)).resourceNode.Usage[val.fr] = val.val
	}
}
