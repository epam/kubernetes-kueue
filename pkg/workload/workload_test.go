...
func TestFromPendingToQuotaReservedOrAdmitted(t *testing.T) {
	cases := map[string]struct {
		prev, new string
		want      bool
	}{
		"pending to quotaReserved":  {StatusPending, StatusQuotaReserved, true},
		"pending to admitted":       {StatusPending, StatusAdmitted, true},
		"pending to pending":        {StatusPending, StatusPending, false},
		"quotaReserved to pending":  {StatusQuotaReserved, StatusPending, false},
		"admitted to pending":       {StatusAdmitted, StatusPending, false},
		"quotaReserved to admitted": {StatusQuotaReserved, StatusAdmitted, false},
		"admitted to quotaReserved": {StatusAdmitted, StatusQuotaReserved, false},
		"pending to finished":       {StatusPending, StatusFinished, false},
		"finished to admitted":      {StatusFinished, StatusAdmitted, false},
		"same quotaReserved":        {StatusQuotaReserved, StatusQuotaReserved, false},
		"same admitted":             {StatusAdmitted, StatusAdmitted, false},
	}
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			if got := FromPendingToQuotaReservedOrAdmitted(tc.prev, tc.new); got != tc.want {
				t.Errorf("FromPendingToQuotaReservedOrAdmitted(%q, %q) = %v, want %v", tc.prev, tc.new, got, tc.want)
			}
		})
	}
}
