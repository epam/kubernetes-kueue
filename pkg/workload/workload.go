...
func SetConditionAndUpdate(ctx context.Context,
	...
	return patching.PatchStatus(ctx, c, wl, client.FieldOwner(managerPrefix+"-"+condition.Type), func(wl *kueue.Workload) (bool, error) {
...
func UpdateReclaimablePods(ctx context.Context, c client.Client, wl *kueue.Workload, reclaimablePods []kueue.ReclaimablePod) error {
	return patching.PatchStatus(ctx, c, wl, constants.ReclaimablePodsMgr, func(wl *kueue.Workload) (bool, error) {
...
func IsAdmissible(w *kueue.Workload) bool {
	return !HasAdmissionGate(w) && !finish.IsFinished(w) && IsActive(w) && !HasQuotaReservation(w)
}
...
func PriorityChanged(log logr.Logger, old, new *kueue.Workload) bool {
	...
	if patching.PriorityClassName(new) != "" &&
		patching.PriorityClassName(old) != patching.PriorityClassName(new) {
		return true
	}
