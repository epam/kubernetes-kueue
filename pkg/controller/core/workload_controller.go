...
func (r *WorkloadReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if workload.IsActive(&wl) {
		...
		var resetRequeueState bool
		if wl.Status.RequeueState != nil {
			resetRequeueState = true
			updated = true
		}
		if updated {
			if evicted {
	...
	// false before the workload's eviction.
	if !workload.IsAdmitted(&wl) {
		err := workload.PatchAdmissionStatus(ctx, r.client, &wl, true, r.clock, func() (bool, error) {
			return workload.SyncAdmittedCondition(&wl, r.clock.Now()), nil
		})
		if err != nil {
			return ctrl.Result{}, client.IgnoreNotFound(err)
		}
