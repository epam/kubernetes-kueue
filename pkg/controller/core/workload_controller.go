	...
	if workload.IsActive(&wl) {
		...
		if updated {
			if evicted {
				...
				if err := workloadevict.Evict(ctx, r.client, r.recorder, &wl, reason, message, underlyingCause, r.clock, exposeLqMetrics, r.roleTracker, r.customLabels, workloadevict.WithCustomPrepare(prepare)); err != nil {
...
func (r *WorkloadReconciler) reconcileNotReadyTimeout(ctx context.Context, req ctrl.Request, wl *kueue.Workload) (time.Duration, error) {
	...
	exposeLqMetrics := r.cache.ShouldExposeLocalQueueMetricsForWorkload(log, wl)
	err = workloadevict.Evict(ctx, r.client, r.recorder, wl, kueue.WorkloadEvictedByPodsReadyTimeout, message, underlyingCause, r.clock, exposeLqMetrics, r.roleTracker, r.customLabels, workloadevict.WithCustomPrepare(func(wl *kueue.Workload) {
