...
func (r *JobReconciler) finalizeWorkloads(ctx context.Context, key types.NamespacedName, job GenericJob) error {
	...
	for i := range workloads {
		wl := &workloads[i]
		if features.Enabled(features.FinishOrphanedWorkloads) && controllerutil.HasControllerReference(wl) && wl.Labels[controllerconsts.JobUIDLabel] != "" {
...
func EquivalentToWorkload(ctx context.Context, c client.Client, job GenericJob, wl *kueue.Workload) (bool, error) {
	...
	if owner.Name != job.Object().GetName() || owner.UID != job.Object().GetUID() {
		return false, nil
	}
