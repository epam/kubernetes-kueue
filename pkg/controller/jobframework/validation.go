...
func validateCreateForPrebuiltWorkload(job GenericJob) field.ErrorList {
	...
	allErrs = validatePrebuiltWorkloadName(job.Object())

	// this rule should be relaxed when its confirmed that running with a prebuilt wl is fully supported by each integration
	if _, hasPrebuilt := PrebuiltWorkloadFor(job.Object()); hasPrebuilt {
		gvk := job.GVK().String()
		if !supportedPrebuiltWlJobGVKs.Has(gvk) {
			allErrs = append(allErrs, field.Forbidden(getPrebuiltWorkloadPath(), fmt.Sprintf("Is not supported for %q", gvk)))
		}
	}
	return allErrs
}
...
func validateUpdateForPrebuiltWorkload(oldJob, newJob GenericJob) field.ErrorList {
	...
	oldWlName, _ := PrebuiltWorkloadFor(oldJob.Object())
	newWlName, _ := PrebuiltWorkloadFor(newJob.Object())
	return apivalidation.ValidateImmutableField(newWlName, oldWlName, getPrebuiltWorkloadPath())
}
