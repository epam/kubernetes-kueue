...
// GenericJob is the required interface that must be implemented by all jobs
...
type GenericJob interface {
	...
	// The message describes the final state, and success indicates whether it succeeded.
...
// JobWithPodLabelSelector is an optional interface that allows retrieving
// the label selector used by pods for the job.
type JobWithPodLabelSelector interface {
	// PodLabelSelector returns the label selector used by pods for the job.
	PodLabelSelector() string
}

// JobWithReclaimablePods is an optional interface that allows retrieving the list of reclaimable pods.
...
// JobWithCustomStop is an optional interface that allows implementing a custom stop procedure.
...
// when custom finalization logic is needed after a job has finished.
...
// JobWithPriorityClass is an optional interface that allows retrieving
// the name of the job's priority class.
...
// ComposableJob is an interface that should be implemented by generic jobs
...
type ComposableJob interface {
	...
	// ConstructComposableWorkload returns a new Workload assembled from all members of the ComposableJob.
	...
	// Stop implements the custom stop procedure for a ComposableJob.
	...
	// EnsureWorkloadOwnedByAllMembers ensures that the workload is owned by all specified members.
...
type JobWithManagedBy interface {
	...
	// SetManagedBy sets the spec field that contains the name
...
// annotation, which reflects the current replica sizes from the underlying
// RayCluster. The job reconciler will call GetCustomAnnotations to update
// such annotations in the API server.
type JobWithCustomAnnotations interface {
	// GetCustomAnnotations returns additional annotations that should be added
	// to the job.
	GetCustomAnnotations(ctx context.Context, c client.Client, podSets []kueue.PodSet) (map[string]string, error)
}

// ElasticWorkloadNameProvider is an optional interface contains method
// to provide extra information to build workload name for elastic job
