...
func validatePrebuiltWorkloadName(pod *Pod) field.ErrorList {
	...
	prebuiltWorkload := jobframework.PrebuiltWorkloadFor(&pod.pod)
	if groupName != "" && prebuiltWorkload != "" && prebuiltWorkload != groupName {
		allErrs = append(allErrs, field.Invalid(getPrebuiltWorkloadPath(&pod.pod), prebuiltWorkload, "prebuilt workload and pod group should be equal"))
	}
	return allErrs
}
...
func getPrebuiltWorkloadPath(obj client.Object) *field.Path {
	prebuiltWorkloadAnnotation := obj.GetAnnotations()[ctrlconstants.PrebuiltWorkloadAnnotation]
