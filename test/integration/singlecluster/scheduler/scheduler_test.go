...
var _ = ginkgo.Describe("Scheduler", func() {
	...
	ginkgo.When("Deleting child Cohort should update borrowable resources for parent Cohort", func() {
		...
		ginkgo.It("Should not admit from stale subtree quota after deleting sibling cohort", func() {
			...
			util.ExpectCohortSubtreeQuotaGaugeMetric(root.GetName(), onDemandFlavor.Name, corev1.ResourceCPU.String(), 22)
			...
			// Wait for the cache to process the cohort deletion by checking the root's subtree quota dropped.
			util.ExpectCohortSubtreeQuotaGaugeMetric(root.GetName(), onDemandFlavor.Name, corev1.ResourceCPU.String(), 10)
