...
var _ = ginkgo.Describe("Workload preemption eviction to pending metrics", ginkgo.Label("controller:workload", "area:core"), ginkgo.Ordered, ginkgo.ContinueOnFailure, func() {
	...
	ginkgo.It("should record preemption_eviction_to_pending_seconds when a preemptee returns to Pending", func() {
		...
		util.ExpectPreemptionEvictionToPendingHistogramMetricAtLeast(kueue.ClusterQueueReference(cq.Name), 1)
	})
})
