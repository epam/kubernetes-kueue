...
var _ = ginkgo.Describe("MultiKueue", func() {
	var (
		...
		kubernetesClients = map[string]struct {
	...
	ginkgo.When("Creating a multikueue integration workload", func() {
		...
		ginkgo.It("Should sync a StatefulSet to worker if admitted", func() {
			...
			admittedWorkerName := waitForJobAdmittedAndGetWorker(wlLookupKey, multiKueueAc.Name)
		...
		ginkgo.It("Should sync a LeaderWorkerSet and run replicas on worker cluster", func() {
			...
			admittedWorkerName := waitForJobAdmittedAndGetWorker(wlLookupKey0, multiKueueAc.Name)
			workerClient := kubernetesClients[admittedWorkerName].client
		...
		ginkgo.It("Should dispatch all LeaderWorkerSet workloads to the same worker", func() {
			...
			admittedWorkerName := waitForJobAdmittedAndGetWorker(wlKeys[0], multiKueueAc.Name)
			workerClient := kubernetesClients[admittedWorkerName].client
		...
		ginkgo.It("Should run a kubeflow PyTorchJob on worker if admitted", func() {
			...
			admittedWorker := waitForJobAdmittedAndGetWorker(wlLookupKey, multiKueueAc.Name)
...
func waitForJobAdmittedAndGetWorker(wlLookupKey types.NamespacedName, acName string) string {
	ginkgo.GinkgoHelper()
	ginkgo.By("Waiting to be admitted ")
	gomega.Eventually(func(g gomega.Gomega) {
		createdWorkload := &kueue.Workload{}
		g.Expect(k8sManagerClient.Get(ctx, wlLookupKey, createdWorkload)).To(gomega.Succeed())
		g.Expect(apimeta.FindStatusCondition(createdWorkload.Status.Conditions, kueue.WorkloadAdmitted)).To(gomega.BeComparableTo(&metav1.Condition{
